...
// Verificar si se ha enviado una solicitud para obtener el reporte de inventario
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['min_cantidad']) && isset($_POST['max_cantidad'])) {
    // Obtener los valores de cantidad del formulario
    $min_cantidad = $_POST['min_cantidad'];
    $max_cantidad = $_POST['max_cantidad'];

    // Preparar la consulta para llamar a la función
    $query = 'BEGIN :cursor := REPORTAR_INVENTARIO_POR_CANTIDAD(:min_cantidad, :max_cantidad); END;';
    ...
    // Bind variables
    oci_bind_by_name($stid, ':min_cantidad', $min_cantidad);
    oci_bind_by_name($stid, ':max_cantidad', $max_cantidad);
...
<head>
    ...
    <title>Reporte de Inventario - Mountain-Bliss-Resort</title>
    ...
    <div class="content">
        ...
        <section class="options_area">
            <div class="container">
                <div class="row">
                    <div class="container">
                        <h1 style="color: #333">Reporte de Inventario por Cantidad</h1>
                        <form method="POST" action="" class="mb-4">
                            <div class="form-group">
                                <label for="min_cantidad">Cantidad Mínima:</label>
                                <input type="number" id="min_cantidad" name="min_cantidad" class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label for="max_cantidad">Cantidad Máxima:</label>
                                <input type="number" id="max_cantidad" name="max_cantidad" class="form-control" required>
                            </div>
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        <?php foreach ($productos as $producto): ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($producto['INVENTARIOID'], ENT_QUOTES); ?></td>
                                                <td><?php echo htmlspecialchars($producto['NOMBREPRODUCTO'], ENT_QUOTES); ?></td>
                                                <td><?php echo htmlspecialchars($producto['CANTIDADTOTAL'], ENT_QUOTES); ?></td>
                                                <td><?php echo htmlspecialchars($producto['UNIDADMEDIDA'], ENT_QUOTES); ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
